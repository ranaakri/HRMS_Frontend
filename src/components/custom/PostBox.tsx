...
import { FaComment, FaEdit, FaHeart, FaTrashRestoreAlt } from "react-icons/fa";
...
export default function PostBox({
  ...
  const restorePost = useMutation({
    mutationFn: async () => {
      ...
      return await api.patch(`/post/restore`, { postId: post.postId, deletedById: user?.userId });
    },
    onSuccess: () => {
      notify.success("Success", "Post restored successfully");
      onDelete?.(post.postId);
    },
    onError: (error: any) => {
      notify.error("Error", error.response?.data?.message);
    ...
    <div className="w-full flex justify-center">
      <div className="gap-4 p-5 md:max-w-200 md:min-w-120 m-5 rounded-2xl bg-white shadow-md w-full">
        <div className="flex items-center gap-3">
          ...
          <div>
            <Link
              to={`/${user?.role.toLocaleLowerCase()}/post/profile-view/${post.author?.userId}`}
              ...
              {post.mentions?.map((mention) => (
                <Tooltip key={mention.email}>
                  <TooltipTrigger asChild>
                    <Link
                      to={`/${user?.role.toLocaleLowerCase()}/post/profile-view/${mention.userId}`}
